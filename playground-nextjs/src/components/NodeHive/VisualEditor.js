...
export default function VisualEditor() {
  ...
  const enableVisualEditing = () => {
    const elements = document.querySelectorAll('[data-nodehive-enable="true"]');
    ...
    elements.forEach((el) => {
      ...
      overlay.onclick = () => openComposableComponent({ uuid, id, parent_id });
  ...
  const openComposableComponent = ({ uuid, id, parent_id }) => {
    ...
    <div className="fixed bottom-8 left-1/2">
      ...
      <div className="-translate-x-1/2 rounded-full bg-zinc-700 p-3">
        <div className="flex gap-2">
          ...
          {/*!isInIframe && ( 
                <button onClick={openbackend} className="flex gap-2 text-white font-bold py-2 px-4 rounded-full">
                    <img src="https://netnode.nodehive.app/sites/default/files/styles/thumbnail/public/2023-09/nodehive-icon.png?itok=Y5yE_YD5" className="w-8 h-8 rounded-full border-2 border-color:white" />
                    <div>Open</div>
                    <div>|</div>
                </button>
                )
                    {!isInIframe && (
                        <button
                            onClick={spacedashboard}
                            className="flex gap-2 rounded rounded-lg bg-blue-500 px-4 py-2 font-bold text-white hover:bg-blue-700"
                        >
                            {' '}
                            <img
                                src="https://netnode.nodehive.app/sites/default/files/styles/thumbnail/public/2023-09/nodehive-icon.png?itok=Y5yE_YD5"
                                width="20"
                                height="20"
                            />
                        </button>
