...
export class AuthManager {
  ...
  async authenticateClientCredentials(options = {}) {
    try {
      ...
      const clientSecret =
        options.clientSecret || this.oauthConfig.clientSecret;
      ...
      const response = await fetch(`${this.client.baseUrl}/oauth/token`, {
        method: "POST",
        headers: {
  ...
  async refreshToken() {
    if (this.authMethod === "nodehive-api-key") {
      throw new AuthenticationError(
        "Token refresh is not needed for NodeHive API Key authentication"
      );
    }
    if (this.authMethod !== "oauth") {
      throw new AuthenticationError(
        "Token refresh is only available for OAuth authentication"
      );
    }
